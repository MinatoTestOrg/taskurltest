...
public class TaskUrlTestTable1BaseService<T extends TaskUrlTestTable1Base> extends BaseBusinessLogic<T>
		implements ITaskUrlTestTable1BaseService<T> {
		
	private static final Logger LOGGER = LoggerFactory.getLogger(TaskUrlTestTable1BaseService.class.getName());
	...
	@Autowired
	private  TaskUrlTestTable1PerimeterBaseImpl taskurltesttable1PerimeterBaseImpl;
	
	
	public TaskUrlTestTable1BaseService(Class<T> modelClass) {
		super(modelClass);
		...
		addPersistenceOption(AnalyticalOptions.GROUP_NAME, "6141e8c1-1cd6-4c48-af86-ff10fc3c1784");
		registerModelTransformer(PersistenceType.ANALYTICAL, new TaskUrlTestTable1AnalyticalTransformer());
	}
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("TaskUrlTestTable1", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("TaskUrlTestTable1", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("TaskUrlTestTable1", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
	...
	protected IPerimeterManager<T> getPerimeterManager() {
		return taskurltesttable1PerimeterBaseImpl;
	}
}