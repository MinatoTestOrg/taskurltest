...
export class Taskurltesttable1DetailBaseComponent{
...
confirmationReference:any;
  ...
  "type" : "form"
	...
	pageViewTitle: string = 'TASKURLTESTTABLE1_DETAIL';
...
step:workflowInfo.step
       ...
       const methodName: any = "configureFormOnWorkflow";
        let action: Exclude<keyof Taskurltesttable1DetailBaseComponent, ' '> = methodName;
...
this.errorMessages = {
 ...
 showConfirmationPopup(config:any){
  ...
  if (environment.prototype && this.id) {
    ...
    this.taskurltesttable1Service.getProtoTypingDataById(params).subscribe((res: any) => {
    ...
    const dataSubscription = this.taskurltesttable1Service.getById (params).subscribe((res: Taskurltesttable1Base[]) => {
      this.data = res || {};
      this.formatRawData();
    });
...
workflowInformation.step = workflowDataForNewRec.step || '';
	...
	editFormula(config:any,fieldname:any){
     ...
     if(fieldConfig.formulaMethod){
       let methodName: Exclude<keyof Taskurltesttable1DetailBaseComponent, ' '> = fieldConfig.formulaMethod
...
if (Array.isArray(ele)) {
  ...
  const requestedObj = this.generateRequestObj(data);
      ...
      const saveSubscription = this.uploadAttachmentsandSaveData(requestedObj, splittedData).subscribe(res => {
        ...
        const saveSubscription = this.taskurltesttable1Service[method](requestedObj).subscribe((res:Taskurltesttable1Base) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
...
uploadAttachmentsandSaveData(data: any, splittedData: any): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.taskurltesttable1Service.create(splittedData.data).subscribe(
        createdData => {
    ...
    const subject$ = new Subject();    
      const updateSubscription = this.deleteFiles(splittedData).subscribe((responseData: any) => {
        this.saveFiles(responseData).subscribe((dataToUpdate: any) => {
          ...
          this.taskurltesttable1Service.update(dataToUpdate.data).subscribe(
            res => {
              subject$.next(res);
              subject$.complete();
            },
